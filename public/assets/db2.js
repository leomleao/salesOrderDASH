...
var mApp = (function() {
	...
	/**
	 * Initializes tables for quadrants
	 */
	var initQuadrantTables = function() {
		mApp.db2Quad = {};
		mApp.db2Quad.m1 = [];
		mApp.db2Quad.m2 = [];
		mApp.db2Quad.m3 = [];
		...
		for (const quadrant in mApp.db2Quad) {
			if (mApp.db2Quad.hasOwnProperty(quadrant)) {
				mApp['db2' + quadrant + 'table'] = $('#m_table_' + quadrant).mDatatable(
					{
						data: {
							type: 'local',
							source: { area: '', name: '', db2: '', sales: '' },
						},
						// layout definition
						layout: {
						...
						columns: [
							...
							},
							{
								...
								template: function(data) {
									return numeral(data.sales).format();
								},
							},
							{
								field: 'db2',
								...
								width: 40,
								template: function(data) {
									return numeral(data.db2 / 100).format('0%');
								},
							},
						],
					},
	...
	var splitIntoQuadrants = function() {
		mApp.db2Data.forEach(customer => {
			...
			if (
				...
				customer.db2 < mApp.db2.target.margin
			) {
				mApp.db2Quad.m1.push(customer);
			}
			//M2
			if (
				customer.sales > mApp.db2.target.sales &&
			...
			if (
				...
				mApp.db2Quad.m4.push(customer);
			}
		});

		for (const quadrant in mApp.db2Quad) {
			if (mApp.db2Quad.hasOwnProperty(quadrant)) {
				const data = mApp.db2Quad[quadrant];

				let options = mApp['db2' + quadrant + 'table'].options;
				options.data.source = data;

				mApp['db2' + quadrant + 'table'].destroy();
				mApp['db2' + quadrant + 'table'] = $('#m_table_' + quadrant).mDatatable(
					options,
				);
			}
	...
	var initDB2Chart = function() {
		...
		mApp.db2Chart = am4core.create('db2Chart', am4charts.XYChart);

		var valueAxisX = mApp.db2Chart.xAxes.push(new am4charts.ValueAxis());
		valueAxisX.renderer.ticks.template.disabled = true;
		valueAxisX.renderer.axisFills.template.disabled = true;

		var valueAxisY = mApp.db2Chart.yAxes.push(new am4charts.ValueAxis());
		valueAxisY.renderer.ticks.template.disabled = true;
		valueAxisY.renderer.axisFills.template.disabled = true;

		var series = mApp.db2Chart.series.push(new am4charts.LineSeries());
		...
		bullet.propertyFields.fill = am4core.color('#8BB63B');
		...
		var outline = mApp.db2Chart.plotContainer.createChild(am4core.Circle);
		...
		bullet.events.on('over', function(event) {
			...
			mApp.db2Chart.cursor.triggerMove({ x: target.pixelX, y: target.pixelY }, 'hard');
			mApp.db2Chart.cursor.lineX.y = target.pixelY;
			mApp.db2Chart.cursor.lineY.x = target.pixelX - mApp.db2Chart.plotContainer.pixelWidth;
		...
		bullet.events.on('out', function(event) {
			mApp.db2Chart.cursor.triggerMove(event.pointer.point, 'none');
			mApp.db2Chart.cursor.lineX.y = 0;
			mApp.db2Chart.cursor.lineY.x = 0;
		...
		mApp.db2Chart.cursor = new am4charts.XYCursor();
		mApp.db2Chart.cursor.behavior = 'zoomXY';

		let cursorPosition = {
			x: null,
			y: null,
		};
		mApp.db2Chart.cursor.events.on('cursorpositionchanged', function(ev) {
		...
		mApp.db2Chart.events.on('doublehit', function(ev) {
			...
			mApp.db2.target = { sales: cursorPosition.x, margin: cursorPosition.y };
		...
		mApp.db2Chart.scrollbarX = new am4core.Scrollbar();
		mApp.db2Chart.scrollbarY = new am4core.Scrollbar();
		mApp.db2Chart.data = mApp.db2Data;
	};
	...
	return {
		 ...
		 *
		 */
		// init quadrant tables
		initQuadrantTables: function() {
			initQuadrantTables();
		},
